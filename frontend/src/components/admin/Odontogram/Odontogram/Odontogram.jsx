...
const Odontogram = ({ initialData = {}, onSave, readOnly = false }) => { 
    ...
    return ( 
        <div className="p-2 sm:p-4 flex flex-col items-center w-full max-w-full overflow-hidden">
            ...
            <div className="w-full max-w-full overflow-x-auto">
                <div className="min-w-max mx-auto px-2">
                    ...
                    <div className={`flex justify-center mb-3 sm:mb-4 ${readOnly ? 'opacity-80' : ''}`}> 
                        {topRowRight.map((t) => ( 
                            <Tooth key={t} label={t} data={toothData[t]} onClick={() => handleToothClick(t)} readOnly={readOnly} /> 
                        ))} 
            ...
            {observations.length > 0 && ( 
                <div className="flex flex-col gap-3 sm:gap-4 w-full max-w-3xl px-2 sm:px-0"> 
                    ...
                    {observations.map((obs, index) => ( 
                        <div key={index} className="bg-white p-3 sm:p-4 rounded-lg sm:rounded-xl shadow-sm sm:shadow-md border border-gray-200 hover:shadow-md sm:hover:shadow-lg transition-shadow w-full h-auto" > 
                            <div className="flex items-center mb-2"> 
                                <div className="w-3 h-3 sm:w-4 sm:h-4 rounded-full mr-2 border" style={{ backgroundColor: obs.color === "blue" ? "#1E3A8A" : obs.color === "red" ? "#B22222" : obs.color === "green" ? "#2E7D32" : "white" }} /> 
